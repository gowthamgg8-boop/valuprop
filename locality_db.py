"""
valUProp.in — Locality Price Database
locality_db.py

AUTO-GENERATED — do not edit manually.
Source: chennai_cma_locality_seed.csv
Generated: 2026-06-28 12:00
Localities: 169 Chennai + 14 Bangalore
RATE_DATA_VERSION: e8199b59-20260628
"""

RATE_DATA_VERSION = "e8199b59-20260628"

from dataclasses import dataclass, field
from typing import Optional

@dataclass
class LocalityData:
    city:              str
    locality:          str
    land_rate_lo:      float
    land_rate_hi:      float
    apt_rate_lo:       float
    apt_rate_hi:       float
    guideline_value:   float
    trend_12m:         str
    micro_context:     str
    infra_notes:       str
    data_confidence:   int   = 75
    demand_drivers:    list  = field(default_factory=list)
    risk_factors:      list  = field(default_factory=list)

LOCALITY_DB: dict[str, LocalityData] = {
    # ── Chennai (169 localities from seed CSV) ──────────────

    "Chennai|Tiruvottiyur": LocalityData(
        city="Chennai", locality="Tiruvottiyur",
        land_rate_lo=5200, land_rate_hi=8550,
        apt_rate_lo=5700,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Tiruvottiyur: North Chennai coastal locality with suburban rail connectivity. Industrial and port-adjacent demand.",
        infra_notes="Pincode 600019. GCC zone. Zone 1 - Thiruvottiyur.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 1 - Thiruvottiyur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kaladipet": LocalityData(
        city="Chennai", locality="Kaladipet",
        land_rate_lo=4950, land_rate_hi=7800,
        apt_rate_lo=5500,  apt_rate_hi=8350,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kaladipet: North Chennai coastal locality with suburban rail connectivity. Industrial and port-adjacent demand.",
        infra_notes="Pincode 600019. GCC zone. Zone 1 - Thiruvottiyur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 1 - Thiruvottiyur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Ennore": LocalityData(
        city="Chennai", locality="Ennore",
        land_rate_lo=3800, land_rate_hi=6200,
        apt_rate_lo=4300,  apt_rate_hi=6650,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Ennore: North Chennai coastal locality with suburban rail connectivity. Industrial and port-adjacent demand.",
        infra_notes="Pincode 600057. GCC zone. Zone 1 - Thiruvottiyur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 1 - Thiruvottiyur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Wimco Nagar": LocalityData(
        city="Chennai", locality="Wimco Nagar",
        land_rate_lo=4000, land_rate_hi=6450,
        apt_rate_lo=4550,  apt_rate_hi=6850,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Wimco Nagar: North Chennai coastal locality with suburban rail connectivity. Industrial and port-adjacent demand.",
        infra_notes="Pincode 600057. GCC zone. Zone 1 - Thiruvottiyur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 1 - Thiruvottiyur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kathivakkam": LocalityData(
        city="Chennai", locality="Kathivakkam",
        land_rate_lo=3600, land_rate_hi=5900,
        apt_rate_lo=4000,  apt_rate_hi=6200,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kathivakkam: North Chennai coastal locality with suburban rail connectivity. Industrial and port-adjacent demand.",
        infra_notes="Pincode 600060. GCC zone. Zone 1 - Thiruvottiyur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 1 - Thiruvottiyur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Manali": LocalityData(
        city="Chennai", locality="Manali",
        land_rate_lo=3600, land_rate_hi=6200,
        apt_rate_lo=4000,  apt_rate_hi=6450,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Manali: North Chennai industrial zone. Petrochemical and manufacturing belt. Limited residential premium.",
        infra_notes="Pincode 600068. GCC zone. Zone 2 - Manali.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 2 - Manali", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Manali New Town": LocalityData(
        city="Chennai", locality="Manali New Town",
        land_rate_lo=3300, land_rate_hi=5700,
        apt_rate_lo=3800,  apt_rate_hi=6200,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Manali New Town: North Chennai industrial zone. Petrochemical and manufacturing belt. Limited residential premium.",
        infra_notes="Pincode 600103. GCC zone. Zone 2 - Manali.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 2 - Manali", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Mathur": LocalityData(
        city="Chennai", locality="Mathur",
        land_rate_lo=3300, land_rate_hi=5500,
        apt_rate_lo=3800,  apt_rate_hi=5900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Mathur: North Chennai industrial zone. Petrochemical and manufacturing belt. Limited residential premium.",
        infra_notes="Pincode 600068. GCC zone. Zone 2 - Manali.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 2 - Manali", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Sathangadu": LocalityData(
        city="Chennai", locality="Sathangadu",
        land_rate_lo=3050, land_rate_hi=5200,
        apt_rate_lo=3600,  apt_rate_hi=5700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Sathangadu: North Chennai industrial zone. Petrochemical and manufacturing belt. Limited residential premium.",
        infra_notes="Pincode 600019. GCC zone. Zone 2 - Manali.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 2 - Manali", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Madhavaram": LocalityData(
        city="Chennai", locality="Madhavaram",
        land_rate_lo=4750, land_rate_hi=7800,
        apt_rate_lo=5200,  apt_rate_hi=8100,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Madhavaram: North Chennai growing corridor. Metro Phase 2 station proposed. Affordable mid-segment demand.",
        infra_notes="Pincode 600060. GCC zone. Zone 3 - Madhavaram.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 3 - Madhavaram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Madhavaram Milk Colony": LocalityData(
        city="Chennai", locality="Madhavaram Milk Colony",
        land_rate_lo=4950, land_rate_hi=8100,
        apt_rate_lo=5500,  apt_rate_hi=8350,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Madhavaram Milk Colony: North Chennai growing corridor. Metro Phase 2 station proposed. Affordable mid-segment demand.",
        infra_notes="Pincode 600051. GCC zone. Zone 3 - Madhavaram.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 3 - Madhavaram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Puzhal": LocalityData(
        city="Chennai", locality="Puzhal",
        land_rate_lo=3300, land_rate_hi=5200,
        apt_rate_lo=3600,  apt_rate_hi=5500,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Puzhal: North Chennai growing corridor. Metro Phase 2 station proposed. Affordable mid-segment demand.",
        infra_notes="Pincode 600066. GCC zone. Zone 3 - Madhavaram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 3 - Madhavaram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kodungaiyur": LocalityData(
        city="Chennai", locality="Kodungaiyur",
        land_rate_lo=4300, land_rate_hi=7100,
        apt_rate_lo=4750,  apt_rate_hi=7400,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kodungaiyur: North Chennai growing corridor. Metro Phase 2 station proposed. Affordable mid-segment demand.",
        infra_notes="Pincode 600118. GCC zone. Zone 3 - Madhavaram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 3 - Madhavaram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Surapet": LocalityData(
        city="Chennai", locality="Surapet",
        land_rate_lo=3800, land_rate_hi=6450,
        apt_rate_lo=4300,  apt_rate_hi=6650,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Surapet: North Chennai growing corridor. Metro Phase 2 station proposed. Affordable mid-segment demand.",
        infra_notes="Pincode 600066. GCC zone. Zone 3 - Madhavaram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 3 - Madhavaram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Tondiarpet": LocalityData(
        city="Chennai", locality="Tondiarpet",
        land_rate_lo=6200, land_rate_hi=10450,
        apt_rate_lo=6650,  apt_rate_hi=10900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Tondiarpet: North Chennai established locality. Port Trust proximity. Mixed residential-commercial.",
        infra_notes="Pincode 600081. GCC zone. Zone 4 - Tondiarpet.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 4 - Tondiarpet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Old Washermanpet": LocalityData(
        city="Chennai", locality="Old Washermanpet",
        land_rate_lo=5500, land_rate_hi=8750,
        apt_rate_lo=5700,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Old Washermanpet: North Chennai established locality. Port Trust proximity. Mixed residential-commercial.",
        infra_notes="Pincode 600021. GCC zone. Zone 4 - Tondiarpet.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 4 - Tondiarpet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Korukkupet": LocalityData(
        city="Chennai", locality="Korukkupet",
        land_rate_lo=5200, land_rate_hi=8350,
        apt_rate_lo=5500,  apt_rate_hi=8550,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Korukkupet: North Chennai established locality. Port Trust proximity. Mixed residential-commercial.",
        infra_notes="Pincode 600021. GCC zone. Zone 4 - Tondiarpet.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 4 - Tondiarpet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Vyasarpadi": LocalityData(
        city="Chennai", locality="Vyasarpadi",
        land_rate_lo=4300, land_rate_hi=7100,
        apt_rate_lo=4750,  apt_rate_hi=7400,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Vyasarpadi: North Chennai established locality. Port Trust proximity. Mixed residential-commercial.",
        infra_notes="Pincode 600039. GCC zone. Zone 4 - Tondiarpet.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 4 - Tondiarpet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Tollgate": LocalityData(
        city="Chennai", locality="Tollgate",
        land_rate_lo=5200, land_rate_hi=8550,
        apt_rate_lo=5700,  apt_rate_hi=8750,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Tollgate: North Chennai established locality. Port Trust proximity. Mixed residential-commercial.",
        infra_notes="Pincode 600021. GCC zone. Zone 4 - Tondiarpet.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 4 - Tondiarpet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Royapuram": LocalityData(
        city="Chennai", locality="Royapuram",
        land_rate_lo=9000, land_rate_hi=15200,
        apt_rate_lo=9500,  apt_rate_hi=15700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Royapuram: North Chennai heritage and commercial zone. Proximity to Chennai Port and George Town.",
        infra_notes="Pincode 600013. GCC zone. Zone 5 - Royapuram.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 5 - Royapuram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Mannady": LocalityData(
        city="Chennai", locality="Mannady",
        land_rate_lo=8550, land_rate_hi=14700,
        apt_rate_lo=8550,  apt_rate_hi=14250,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Mannady: North Chennai heritage and commercial zone. Proximity to Chennai Port and George Town.",
        infra_notes="Pincode 600001. GCC zone. Zone 5 - Royapuram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 5 - Royapuram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Parrys": LocalityData(
        city="Chennai", locality="Parrys",
        land_rate_lo=8100, land_rate_hi=14250,
        apt_rate_lo=8100,  apt_rate_hi=13800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Parrys: North Chennai heritage and commercial zone. Proximity to Chennai Port and George Town.",
        infra_notes="Pincode 600001. GCC zone. Zone 5 - Royapuram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 5 - Royapuram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Sowcarpet": LocalityData(
        city="Chennai", locality="Sowcarpet",
        land_rate_lo=9500, land_rate_hi=16600,
        apt_rate_lo=9000,  apt_rate_hi=15700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Sowcarpet: North Chennai heritage and commercial zone. Proximity to Chennai Port and George Town.",
        infra_notes="Pincode 600079. GCC zone. Zone 5 - Royapuram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 5 - Royapuram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Washermanpet": LocalityData(
        city="Chennai", locality="Washermanpet",
        land_rate_lo=5700, land_rate_hi=9500,
        apt_rate_lo=6200,  apt_rate_hi=10000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Washermanpet: North Chennai heritage and commercial zone. Proximity to Chennai Port and George Town.",
        infra_notes="Pincode 600021. GCC zone. Zone 5 - Royapuram.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 5 - Royapuram", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Perambur": LocalityData(
        city="Chennai", locality="Perambur",
        land_rate_lo=5700, land_rate_hi=9000,
        apt_rate_lo=6200,  apt_rate_hi=9500,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Perambur: Mid-Chennai established residential. Perambur metro connectivity. Mid-segment demand.",
        infra_notes="Pincode 600011. GCC zone. Zone 6 - Thiru Vi Ka Nagar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 6 - Thiru Vi Ka Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Ayanavaram": LocalityData(
        city="Chennai", locality="Ayanavaram",
        land_rate_lo=6650, land_rate_hi=10450,
        apt_rate_lo=6650,  apt_rate_hi=10000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Ayanavaram: Mid-Chennai established residential. Perambur metro connectivity. Mid-segment demand.",
        infra_notes="Pincode 600023. GCC zone. Zone 6 - Thiru Vi Ka Nagar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 6 - Thiru Vi Ka Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Pulianthope": LocalityData(
        city="Chennai", locality="Pulianthope",
        land_rate_lo=4950, land_rate_hi=7800,
        apt_rate_lo=5200,  apt_rate_hi=8100,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Pulianthope: Mid-Chennai established residential. Perambur metro connectivity. Mid-segment demand.",
        infra_notes="Pincode 600012. GCC zone. Zone 6 - Thiru Vi Ka Nagar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 6 - Thiru Vi Ka Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Otteri": LocalityData(
        city="Chennai", locality="Otteri",
        land_rate_lo=6200, land_rate_hi=10000,
        apt_rate_lo=6200,  apt_rate_hi=9500,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Otteri: Mid-Chennai established residential. Perambur metro connectivity. Mid-segment demand.",
        infra_notes="Pincode 600012. GCC zone. Zone 6 - Thiru Vi Ka Nagar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 6 - Thiru Vi Ka Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kosapet": LocalityData(
        city="Chennai", locality="Kosapet",
        land_rate_lo=5900, land_rate_hi=9500,
        apt_rate_lo=6200,  apt_rate_hi=9500,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kosapet: Mid-Chennai established residential. Perambur metro connectivity. Mid-segment demand.",
        infra_notes="Pincode 600012. GCC zone. Zone 6 - Thiru Vi Ka Nagar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 6 - Thiru Vi Ka Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Choolai": LocalityData(
        city="Chennai", locality="Choolai",
        land_rate_lo=6200, land_rate_hi=10000,
        apt_rate_lo=6450,  apt_rate_hi=10000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Choolai: Mid-Chennai established residential. Perambur metro connectivity. Mid-segment demand.",
        infra_notes="Pincode 600112. GCC zone. Zone 6 - Thiru Vi Ka Nagar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 6 - Thiru Vi Ka Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Sembium": LocalityData(
        city="Chennai", locality="Sembium",
        land_rate_lo=5500, land_rate_hi=8750,
        apt_rate_lo=5700,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Sembium: Mid-Chennai established residential. Perambur metro connectivity. Mid-segment demand.",
        infra_notes="Pincode 600011. GCC zone. Zone 6 - Thiru Vi Ka Nagar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 6 - Thiru Vi Ka Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Erukkancherry": LocalityData(
        city="Chennai", locality="Erukkancherry",
        land_rate_lo=4300, land_rate_hi=6850,
        apt_rate_lo=4550,  apt_rate_hi=7100,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Erukkancherry: Mid-Chennai established residential. Perambur metro connectivity. Mid-segment demand.",
        infra_notes="Pincode 600118. GCC zone. Zone 6 - Thiru Vi Ka Nagar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 6 - Thiru Vi Ka Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Ambattur": LocalityData(
        city="Chennai", locality="Ambattur",
        land_rate_lo=4550, land_rate_hi=6850,
        apt_rate_lo=4750,  apt_rate_hi=7100,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Ambattur: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600053. GCC zone. Zone 7 - Ambattur.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Korattur": LocalityData(
        city="Chennai", locality="Korattur",
        land_rate_lo=5700, land_rate_hi=8550,
        apt_rate_lo=6200,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Korattur: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600080. GCC zone. Zone 7 - Ambattur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Mogappair": LocalityData(
        city="Chennai", locality="Mogappair",
        land_rate_lo=6650, land_rate_hi=10450,
        apt_rate_lo=7100,  apt_rate_hi=10450,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Mogappair: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600037. GCC zone. Zone 7 - Ambattur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Mogappair West": LocalityData(
        city="Chennai", locality="Mogappair West",
        land_rate_lo=6450, land_rate_hi=10000,
        apt_rate_lo=6650,  apt_rate_hi=10000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Mogappair West: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600037. GCC zone. Zone 7 - Ambattur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Nolambur": LocalityData(
        city="Chennai", locality="Nolambur",
        land_rate_lo=6200, land_rate_hi=9300,
        apt_rate_lo=6650,  apt_rate_hi=9500,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Nolambur: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600095. GCC zone. Zone 7 - Ambattur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Padi": LocalityData(
        city="Chennai", locality="Padi",
        land_rate_lo=5200, land_rate_hi=8350,
        apt_rate_lo=5700,  apt_rate_hi=8550,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Padi: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600050. GCC zone. Zone 7 - Ambattur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Anna Nagar West Extn": LocalityData(
        city="Chennai", locality="Anna Nagar West Extn",
        land_rate_lo=7600, land_rate_hi=12350,
        apt_rate_lo=8100,  apt_rate_hi=12350,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Anna Nagar West Extn: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600101. GCC zone. Zone 7 - Ambattur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Ayappakkam": LocalityData(
        city="Chennai", locality="Ayappakkam",
        land_rate_lo=4000, land_rate_hi=6450,
        apt_rate_lo=4300,  apt_rate_hi=6650,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Ayappakkam: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600077. GCC zone. Zone 7 - Ambattur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Anna Nagar": LocalityData(
        city="Chennai", locality="Anna Nagar",
        land_rate_lo=12350, land_rate_hi=19950,
        apt_rate_lo=11400,  apt_rate_hi=17100,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Anna Nagar: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600040. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Anna Nagar West": LocalityData(
        city="Chennai", locality="Anna Nagar West",
        land_rate_lo=10450, land_rate_hi=16150,
        apt_rate_lo=9500,  apt_rate_hi=13800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Anna Nagar West: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600101. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Shenoy Nagar": LocalityData(
        city="Chennai", locality="Shenoy Nagar",
        land_rate_lo=10450, land_rate_hi=15200,
        apt_rate_lo=9500,  apt_rate_hi=13800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Shenoy Nagar: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600030. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Aminjikarai": LocalityData(
        city="Chennai", locality="Aminjikarai",
        land_rate_lo=8550, land_rate_hi=13300,
        apt_rate_lo=8100,  apt_rate_hi=11900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Aminjikarai: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600029. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Villivakkam": LocalityData(
        city="Chennai", locality="Villivakkam",
        land_rate_lo=7600, land_rate_hi=12350,
        apt_rate_lo=6650,  apt_rate_hi=10000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Villivakkam: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600049. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kolathur": LocalityData(
        city="Chennai", locality="Kolathur",
        land_rate_lo=4750, land_rate_hi=7400,
        apt_rate_lo=5200,  apt_rate_hi=7600,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kolathur: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600099. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Purasawalkam": LocalityData(
        city="Chennai", locality="Purasawalkam",
        land_rate_lo=11400, land_rate_hi=18050,
        apt_rate_lo=10450,  apt_rate_hi=16600,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Purasawalkam: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600084. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Vepery": LocalityData(
        city="Chennai", locality="Vepery",
        land_rate_lo=9500, land_rate_hi=16150,
        apt_rate_lo=9500,  apt_rate_hi=15200,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Vepery: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600007. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Periyar Nagar": LocalityData(
        city="Chennai", locality="Periyar Nagar",
        land_rate_lo=6200, land_rate_hi=10000,
        apt_rate_lo=6200,  apt_rate_hi=9500,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Periyar Nagar: Premium North-West Chennai. Anna Nagar metro (Green Line). Top schools, hospitals, and retail.",
        infra_notes="Pincode 600082. GCC zone. Zone 8 - Anna Nagar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 8 - Anna Nagar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Teynampet": LocalityData(
        city="Chennai", locality="Teynampet",
        land_rate_lo=16150, land_rate_hi=26600,
        apt_rate_lo=14250,  apt_rate_hi=21850,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Teynampet: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600018. GCC zone. Zone 9 - Teynampet.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Alwarpet": LocalityData(
        city="Chennai", locality="Alwarpet",
        land_rate_lo=19000, land_rate_hi=33250,
        apt_rate_lo=17100,  apt_rate_hi=24700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Alwarpet: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600018. GCC zone. Zone 9 - Teynampet.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Nungambakkam": LocalityData(
        city="Chennai", locality="Nungambakkam",
        land_rate_lo=17100, land_rate_hi=28500,
        apt_rate_lo=14250,  apt_rate_hi=20900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Nungambakkam: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600034. GCC zone. Zone 9 - Teynampet.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|T Nagar": LocalityData(
        city="Chennai", locality="T Nagar",
        land_rate_lo=17100, land_rate_hi=26600,
        apt_rate_lo=15200,  apt_rate_hi=19950,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="T Nagar: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600017. GCC zone. Zone 9 - Teynampet.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Chetpet": LocalityData(
        city="Chennai", locality="Chetpet",
        land_rate_lo=12350, land_rate_hi=19000,
        apt_rate_lo=11400,  apt_rate_hi=16600,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Chetpet: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600031. GCC zone. Zone 9 - Teynampet.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Gopalapuram": LocalityData(
        city="Chennai", locality="Gopalapuram",
        land_rate_lo=17100, land_rate_hi=28500,
        apt_rate_lo=15200,  apt_rate_hi=22800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Gopalapuram: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600086. GCC zone. Zone 9 - Teynampet.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Egmore": LocalityData(
        city="Chennai", locality="Egmore",
        land_rate_lo=11400, land_rate_hi=18050,
        apt_rate_lo=10450,  apt_rate_hi=15700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Egmore: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600008. GCC zone. Zone 9 - Teynampet.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Triplicane": LocalityData(
        city="Chennai", locality="Triplicane",
        land_rate_lo=8100, land_rate_hi=13800,
        apt_rate_lo=7600,  apt_rate_hi=12350,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Triplicane: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600005. GCC zone. Zone 9 - Teynampet.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Royapettah": LocalityData(
        city="Chennai", locality="Royapettah",
        land_rate_lo=11400, land_rate_hi=18050,
        apt_rate_lo=10450,  apt_rate_hi=15200,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Royapettah: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600014. GCC zone. Zone 9 - Teynampet.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Thousand Lights": LocalityData(
        city="Chennai", locality="Thousand Lights",
        land_rate_lo=15200, land_rate_hi=24700,
        apt_rate_lo=13300,  apt_rate_hi=19950,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Thousand Lights: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600006. GCC zone. Zone 9 - Teynampet.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kodambakkam": LocalityData(
        city="Chennai", locality="Kodambakkam",
        land_rate_lo=9500, land_rate_hi=15200,
        apt_rate_lo=8550,  apt_rate_hi=12800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kodambakkam: West Chennai established. Good metro access. Strong school and hospital belt.",
        infra_notes="Pincode 600024. GCC zone. Zone 10 - Kodambakkam.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 10 - Kodambakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Vadapalani": LocalityData(
        city="Chennai", locality="Vadapalani",
        land_rate_lo=9500, land_rate_hi=15200,
        apt_rate_lo=9000,  apt_rate_hi=13300,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Vadapalani: West Chennai established. Good metro access. Strong school and hospital belt.",
        infra_notes="Pincode 600026. GCC zone. Zone 10 - Kodambakkam.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 10 - Kodambakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Ashok Nagar": LocalityData(
        city="Chennai", locality="Ashok Nagar",
        land_rate_lo=10450, land_rate_hi=16150,
        apt_rate_lo=9500,  apt_rate_hi=13800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Ashok Nagar: West Chennai established. Good metro access. Strong school and hospital belt.",
        infra_notes="Pincode 600083. GCC zone. Zone 10 - Kodambakkam.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 10 - Kodambakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|KK Nagar": LocalityData(
        city="Chennai", locality="KK Nagar",
        land_rate_lo=9500, land_rate_hi=15200,
        apt_rate_lo=8550,  apt_rate_hi=12800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="KK Nagar: West Chennai established. Good metro access. Strong school and hospital belt.",
        infra_notes="Pincode 600078. GCC zone. Zone 10 - Kodambakkam.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 10 - Kodambakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Saligramam": LocalityData(
        city="Chennai", locality="Saligramam",
        land_rate_lo=8550, land_rate_hi=13300,
        apt_rate_lo=8100,  apt_rate_hi=12350,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Saligramam: West Chennai established. Good metro access. Strong school and hospital belt.",
        infra_notes="Pincode 600093. GCC zone. Zone 10 - Kodambakkam.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 10 - Kodambakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Choolaimedu": LocalityData(
        city="Chennai", locality="Choolaimedu",
        land_rate_lo=8100, land_rate_hi=12800,
        apt_rate_lo=7600,  apt_rate_hi=10900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Choolaimedu: West Chennai established. Good metro access. Strong school and hospital belt.",
        infra_notes="Pincode 600094. GCC zone. Zone 10 - Kodambakkam.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 10 - Kodambakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Nesapakkam": LocalityData(
        city="Chennai", locality="Nesapakkam",
        land_rate_lo=8100, land_rate_hi=12800,
        apt_rate_lo=7600,  apt_rate_hi=10900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Nesapakkam: West Chennai established. Good metro access. Strong school and hospital belt.",
        infra_notes="Pincode 600078. GCC zone. Zone 10 - Kodambakkam.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 10 - Kodambakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|MGR Nagar": LocalityData(
        city="Chennai", locality="MGR Nagar",
        land_rate_lo=7600, land_rate_hi=12350,
        apt_rate_lo=7100,  apt_rate_hi=10450,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="MGR Nagar: West Chennai established. Good metro access. Strong school and hospital belt.",
        infra_notes="Pincode 600078. GCC zone. Zone 10 - Kodambakkam.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 10 - Kodambakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Valasaravakkam": LocalityData(
        city="Chennai", locality="Valasaravakkam",
        land_rate_lo=6650, land_rate_hi=10450,
        apt_rate_lo=6650,  apt_rate_hi=10000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Valasaravakkam: West Chennai residential corridor. NH-48 access. IT park proximity (Porur, Ramapuram).",
        infra_notes="Pincode 600087. GCC zone. Zone 11 - Valasaravakkam.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 11 - Valasaravakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Porur": LocalityData(
        city="Chennai", locality="Porur",
        land_rate_lo=6650, land_rate_hi=10900,
        apt_rate_lo=7100,  apt_rate_hi=10450,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Porur: West Chennai residential corridor. NH-48 access. IT park proximity (Porur, Ramapuram).",
        infra_notes="Pincode 600116. GCC zone. Zone 11 - Valasaravakkam.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 11 - Valasaravakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Ramapuram": LocalityData(
        city="Chennai", locality="Ramapuram",
        land_rate_lo=6450, land_rate_hi=10000,
        apt_rate_lo=6650,  apt_rate_hi=10000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Ramapuram: West Chennai residential corridor. NH-48 access. IT park proximity (Porur, Ramapuram).",
        infra_notes="Pincode 600089. GCC zone. Zone 11 - Valasaravakkam.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 11 - Valasaravakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Maduravoyal": LocalityData(
        city="Chennai", locality="Maduravoyal",
        land_rate_lo=5700, land_rate_hi=9000,
        apt_rate_lo=6200,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Maduravoyal: West Chennai residential corridor. NH-48 access. IT park proximity (Porur, Ramapuram).",
        infra_notes="Pincode 600095. GCC zone. Zone 11 - Valasaravakkam.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 11 - Valasaravakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Virugambakkam": LocalityData(
        city="Chennai", locality="Virugambakkam",
        land_rate_lo=8100, land_rate_hi=12800,
        apt_rate_lo=7600,  apt_rate_hi=11400,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Virugambakkam: West Chennai residential corridor. NH-48 access. IT park proximity (Porur, Ramapuram).",
        infra_notes="Pincode 600092. GCC zone. Zone 11 - Valasaravakkam.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 11 - Valasaravakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Alwarthirunagar": LocalityData(
        city="Chennai", locality="Alwarthirunagar",
        land_rate_lo=6650, land_rate_hi=10450,
        apt_rate_lo=6650,  apt_rate_hi=10000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Alwarthirunagar: West Chennai residential corridor. NH-48 access. IT park proximity (Porur, Ramapuram).",
        infra_notes="Pincode 600087. GCC zone. Zone 11 - Valasaravakkam.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 11 - Valasaravakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Iyyappanthangal": LocalityData(
        city="Chennai", locality="Iyyappanthangal",
        land_rate_lo=4750, land_rate_hi=7600,
        apt_rate_lo=5200,  apt_rate_hi=8100,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Iyyappanthangal: West Chennai residential corridor. NH-48 access. IT park proximity (Porur, Ramapuram).",
        infra_notes="Pincode 600056. GCC zone. Zone 11 - Valasaravakkam.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 11 - Valasaravakkam", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Alandur": LocalityData(
        city="Chennai", locality="Alandur",
        land_rate_lo=9000, land_rate_hi=14700,
        apt_rate_lo=8550,  apt_rate_hi=12800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Alandur: South Chennai GCC zone. Airport adjacent. Good MRTS and GST Road access.",
        infra_notes="Pincode 600016. GCC zone. Zone 12 - Alandur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 12 - Alandur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Nanganallur": LocalityData(
        city="Chennai", locality="Nanganallur",
        land_rate_lo=7600, land_rate_hi=12350,
        apt_rate_lo=7600,  apt_rate_hi=11400,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Nanganallur: South Chennai GCC zone. Airport adjacent. Good MRTS and GST Road access.",
        infra_notes="Pincode 600061. GCC zone. Zone 12 - Alandur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 12 - Alandur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Pazhavanthangal": LocalityData(
        city="Chennai", locality="Pazhavanthangal",
        land_rate_lo=7600, land_rate_hi=12350,
        apt_rate_lo=7600,  apt_rate_hi=11400,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Pazhavanthangal: South Chennai GCC zone. Airport adjacent. Good MRTS and GST Road access.",
        infra_notes="Pincode 600114. GCC zone. Zone 12 - Alandur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 12 - Alandur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|St Thomas Mount": LocalityData(
        city="Chennai", locality="St Thomas Mount",
        land_rate_lo=8100, land_rate_hi=12800,
        apt_rate_lo=8100,  apt_rate_hi=11900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="St Thomas Mount: South Chennai GCC zone. Airport adjacent. Good MRTS and GST Road access.",
        infra_notes="Pincode 600016. GCC zone. Zone 12 - Alandur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 12 - Alandur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Meenambakkam": LocalityData(
        city="Chennai", locality="Meenambakkam",
        land_rate_lo=6200, land_rate_hi=9500,
        apt_rate_lo=6200,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Meenambakkam: South Chennai GCC zone. Airport adjacent. Good MRTS and GST Road access.",
        infra_notes="Pincode 600027. GCC zone. Zone 12 - Alandur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 12 - Alandur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Adambakkam": LocalityData(
        city="Chennai", locality="Adambakkam",
        land_rate_lo=7100, land_rate_hi=11400,
        apt_rate_lo=7100,  apt_rate_hi=10450,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Adambakkam: South Chennai GCC zone. Airport adjacent. Good MRTS and GST Road access.",
        infra_notes="Pincode 600088. GCC zone. Zone 12 - Alandur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 12 - Alandur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Adyar": LocalityData(
        city="Chennai", locality="Adyar",
        land_rate_lo=14250, land_rate_hi=20900,
        apt_rate_lo=13800,  apt_rate_hi=18050,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Adyar: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600020. GCC zone. Zone 13 - Adyar.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Besant Nagar": LocalityData(
        city="Chennai", locality="Besant Nagar",
        land_rate_lo=15200, land_rate_hi=21850,
        apt_rate_lo=13300,  apt_rate_hi=17600,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Besant Nagar: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600090. GCC zone. Zone 13 - Adyar.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Thiruvanmiyur": LocalityData(
        city="Chennai", locality="Thiruvanmiyur",
        land_rate_lo=11400, land_rate_hi=17100,
        apt_rate_lo=11400,  apt_rate_hi=15700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Thiruvanmiyur: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600041. GCC zone. Zone 13 - Adyar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kottivakkam": LocalityData(
        city="Chennai", locality="Kottivakkam",
        land_rate_lo=11400, land_rate_hi=18050,
        apt_rate_lo=10450,  apt_rate_hi=14700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kottivakkam: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600041. GCC zone. Zone 13 - Adyar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Palavakkam": LocalityData(
        city="Chennai", locality="Palavakkam",
        land_rate_lo=12350, land_rate_hi=19000,
        apt_rate_lo=10450,  apt_rate_hi=15200,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Palavakkam: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600041. GCC zone. Zone 13 - Adyar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Neelankarai": LocalityData(
        city="Chennai", locality="Neelankarai",
        land_rate_lo=11400, land_rate_hi=20900,
        apt_rate_lo=9500,  apt_rate_hi=14700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Neelankarai: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600115. GCC zone. Zone 13 - Adyar.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Injambakkam": LocalityData(
        city="Chennai", locality="Injambakkam",
        land_rate_lo=9500, land_rate_hi=17100,
        apt_rate_lo=9000,  apt_rate_hi=13300,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Injambakkam: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600115. GCC zone. Zone 13 - Adyar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Velachery": LocalityData(
        city="Chennai", locality="Velachery",
        land_rate_lo=7600, land_rate_hi=11400,
        apt_rate_lo=7450,  apt_rate_hi=12550,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Velachery: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600042. GCC zone. Zone 13 - Adyar.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Guindy": LocalityData(
        city="Chennai", locality="Guindy",
        land_rate_lo=8550, land_rate_hi=13300,
        apt_rate_lo=8550,  apt_rate_hi=12800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Guindy: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600032. GCC zone. Zone 13 - Adyar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kotturpuram": LocalityData(
        city="Chennai", locality="Kotturpuram",
        land_rate_lo=12350, land_rate_hi=19000,
        apt_rate_lo=11400,  apt_rate_hi=16600,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kotturpuram: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600085. GCC zone. Zone 13 - Adyar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Saidapet": LocalityData(
        city="Chennai", locality="Saidapet",
        land_rate_lo=8550, land_rate_hi=13300,
        apt_rate_lo=7600,  apt_rate_hi=11900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Saidapet: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600015. GCC zone. Zone 13 - Adyar.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Akkarai": LocalityData(
        city="Chennai", locality="Akkarai",
        land_rate_lo=10450, land_rate_hi=18050,
        apt_rate_lo=9000,  apt_rate_hi=13300,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Akkarai: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600119. GCC zone. Zone 13 - Adyar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Uthandi": LocalityData(
        city="Chennai", locality="Uthandi",
        land_rate_lo=7600, land_rate_hi=14250,
        apt_rate_lo=7600,  apt_rate_hi=11900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Uthandi: Premium South Chennai. Beach proximity, Theosophical Society. HNI and premium demand.",
        infra_notes="Pincode 600119. GCC zone. Zone 13 - Adyar.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 13 - Adyar", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Perungudi": LocalityData(
        city="Chennai", locality="Perungudi",
        land_rate_lo=6200, land_rate_hi=8100,
        apt_rate_lo=12800,  apt_rate_hi=17100,
        guideline_value=4400,
        trend_12m="+5.5%",
        micro_context="Perungudi: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600096. GCC zone. Zone 14 - Perungudi.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Thoraipakkam": LocalityData(
        city="Chennai", locality="Thoraipakkam",
        land_rate_lo=5700, land_rate_hi=7400,
        apt_rate_lo=10450,  apt_rate_hi=14700,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Thoraipakkam: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600097. GCC zone. Zone 14 - Perungudi.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Karapakkam": LocalityData(
        city="Chennai", locality="Karapakkam",
        land_rate_lo=5700, land_rate_hi=7800,
        apt_rate_lo=9000,  apt_rate_hi=12350,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Karapakkam: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600097. GCC zone. Zone 14 - Perungudi.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Pallikaranai": LocalityData(
        city="Chennai", locality="Pallikaranai",
        land_rate_lo=5200, land_rate_hi=7400,
        apt_rate_lo=6200,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Pallikaranai: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600100. GCC zone. Zone 14 - Perungudi.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Madipakkam": LocalityData(
        city="Chennai", locality="Madipakkam",
        land_rate_lo=5200, land_rate_hi=7400,
        apt_rate_lo=5700,  apt_rate_hi=8350,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Madipakkam: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600091. GCC zone. Zone 14 - Perungudi.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Medavakkam": LocalityData(
        city="Chennai", locality="Medavakkam",
        land_rate_lo=4300, land_rate_hi=6450,
        apt_rate_lo=5200,  apt_rate_hi=7800,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Medavakkam: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600100. GCC zone. Zone 14 - Perungudi.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Keelkattalai": LocalityData(
        city="Chennai", locality="Keelkattalai",
        land_rate_lo=4550, land_rate_hi=6450,
        apt_rate_lo=5500,  apt_rate_hi=7600,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Keelkattalai: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600117. GCC zone. Zone 14 - Perungudi.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Kovilambakkam": LocalityData(
        city="Chennai", locality="Kovilambakkam",
        land_rate_lo=4300, land_rate_hi=6200,
        apt_rate_lo=5200,  apt_rate_hi=7400,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Kovilambakkam: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600129. GCC zone. Zone 14 - Perungudi.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Taramani": LocalityData(
        city="Chennai", locality="Taramani",
        land_rate_lo=6650, land_rate_hi=9500,
        apt_rate_lo=9500,  apt_rate_hi=14250,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Taramani: OMR IT corridor. Premium apartments, IT park demand. High appreciation zone.",
        infra_notes="Pincode 600113. GCC zone. Zone 14 - Perungudi.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 14 - Perungudi", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Sholinganallur": LocalityData(
        city="Chennai", locality="Sholinganallur",
        land_rate_lo=5500, land_rate_hi=7400,
        apt_rate_lo=8100,  apt_rate_hi=11900,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Sholinganallur: OMR South. Active IT corridor. Metro Phase 2 planned. Strong appreciation.",
        infra_notes="Pincode 600119. GCC zone. Zone 15 - Sholinganallur.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 15 - Sholinganallur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Perumbakkam": LocalityData(
        city="Chennai", locality="Perumbakkam",
        land_rate_lo=5200, land_rate_hi=7100,
        apt_rate_lo=6200,  apt_rate_hi=8750,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Perumbakkam: OMR South. Active IT corridor. Metro Phase 2 planned. Strong appreciation.",
        infra_notes="Pincode 600100. GCC zone. Zone 15 - Sholinganallur.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 15 - Sholinganallur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Semmancheri": LocalityData(
        city="Chennai", locality="Semmancheri",
        land_rate_lo=3800, land_rate_hi=5500,
        apt_rate_lo=5200,  apt_rate_hi=7400,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Semmancheri: OMR South. Active IT corridor. Metro Phase 2 planned. Strong appreciation.",
        infra_notes="Pincode 600119. GCC zone. Zone 15 - Sholinganallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 15 - Sholinganallur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Navalur": LocalityData(
        city="Chennai", locality="Navalur",
        land_rate_lo=5700, land_rate_hi=8100,
        apt_rate_lo=6450,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Navalur: OMR South. Active IT corridor. Metro Phase 2 planned. Strong appreciation.",
        infra_notes="Pincode 600130. GCC zone. Zone 15 - Sholinganallur.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 15 - Sholinganallur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Siruseri": LocalityData(
        city="Chennai", locality="Siruseri",
        land_rate_lo=4000, land_rate_hi=5700,
        apt_rate_lo=5700,  apt_rate_hi=8100,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Siruseri: OMR South. Active IT corridor. Metro Phase 2 planned. Strong appreciation.",
        infra_notes="Pincode 603103. GCC zone. Zone 15 - Sholinganallur.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Zone 15 - Sholinganallur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Sithalapakkam": LocalityData(
        city="Chennai", locality="Sithalapakkam",
        land_rate_lo=3800, land_rate_hi=5500,
        apt_rate_lo=4750,  apt_rate_hi=6850,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Sithalapakkam: OMR South. Active IT corridor. Metro Phase 2 planned. Strong appreciation.",
        infra_notes="Pincode 600126. GCC zone. Zone 15 - Sholinganallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 15 - Sholinganallur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Avadi": LocalityData(
        city="Chennai", locality="Avadi",
        land_rate_lo=3600, land_rate_hi=5700,
        apt_rate_lo=3800,  apt_rate_hi=5900,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Avadi: Avadi Corp zone. North-West suburban belt. Affordable to mid-segment. Defence and SME workforce.",
        infra_notes="Pincode 600054. Avadi Corp zone. Avadi Corporation.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Avadi Corporation", "Chennai growth"],
        risk_factors=["Verify Avadi Corp approvals before purchase"],
    ),

    "Chennai|Pattabiram": LocalityData(
        city="Chennai", locality="Pattabiram",
        land_rate_lo=3300, land_rate_hi=5350,
        apt_rate_lo=3600,  apt_rate_hi=5500,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Pattabiram: Avadi Corp zone. North-West suburban belt. Affordable to mid-segment. Defence and SME workforce.",
        infra_notes="Pincode 600072. Avadi Corp zone. Avadi Corporation.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Avadi Corporation", "Chennai growth"],
        risk_factors=["Verify Avadi Corp approvals before purchase"],
    ),

    "Chennai|Thirumullaivoyal": LocalityData(
        city="Chennai", locality="Thirumullaivoyal",
        land_rate_lo=4300, land_rate_hi=6200,
        apt_rate_lo=4200,  apt_rate_hi=5900,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Thirumullaivoyal: Avadi Corp zone. North-West suburban belt. Affordable to mid-segment. Defence and SME workforce.",
        infra_notes="Pincode 600062. Avadi Corp zone. Avadi Corporation.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Avadi Corporation", "Chennai growth"],
        risk_factors=["Verify Avadi Corp approvals before purchase"],
    ),

    "Chennai|Ambattur OT": LocalityData(
        city="Chennai", locality="Ambattur OT",
        land_rate_lo=4000, land_rate_hi=6450,
        apt_rate_lo=4300,  apt_rate_hi=6650,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Ambattur OT: West Chennai industrial-residential corridor. Ambattur Industrial Estate employment anchor.",
        infra_notes="Pincode 600053. GCC zone. Zone 7 - Ambattur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 7 - Ambattur", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Paruthipattu": LocalityData(
        city="Chennai", locality="Paruthipattu",
        land_rate_lo=3450, land_rate_hi=5500,
        apt_rate_lo=3800,  apt_rate_hi=5700,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Paruthipattu: Avadi Corp zone. North-West suburban belt. Affordable to mid-segment. Defence and SME workforce.",
        infra_notes="Pincode 600071. Avadi Corp zone. Avadi Corporation.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Avadi Corporation", "Chennai growth"],
        risk_factors=["Verify Avadi Corp approvals before purchase"],
    ),

    "Chennai|Kamaraj Nagar Avadi": LocalityData(
        city="Chennai", locality="Kamaraj Nagar Avadi",
        land_rate_lo=3450, land_rate_hi=5500,
        apt_rate_lo=3800,  apt_rate_hi=5700,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Kamaraj Nagar Avadi: Avadi Corp zone. North-West suburban belt. Affordable to mid-segment. Defence and SME workforce.",
        infra_notes="Pincode 600071. Avadi Corp zone. Avadi Corporation.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Avadi Corporation", "Chennai growth"],
        risk_factors=["Verify Avadi Corp approvals before purchase"],
    ),

    "Chennai|Tambaram": LocalityData(
        city="Chennai", locality="Tambaram",
        land_rate_lo=4300, land_rate_hi=7400,
        apt_rate_lo=4750,  apt_rate_hi=7600,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Tambaram: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600045. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Tambaram West": LocalityData(
        city="Chennai", locality="Tambaram West",
        land_rate_lo=4000, land_rate_hi=6650,
        apt_rate_lo=4550,  apt_rate_hi=6850,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Tambaram West: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600045. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|East Tambaram": LocalityData(
        city="Chennai", locality="East Tambaram",
        land_rate_lo=4000, land_rate_hi=6650,
        apt_rate_lo=4550,  apt_rate_hi=6850,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="East Tambaram: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600059. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Selaiyur": LocalityData(
        city="Chennai", locality="Selaiyur",
        land_rate_lo=4300, land_rate_hi=6850,
        apt_rate_lo=4750,  apt_rate_hi=7100,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Selaiyur: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600073. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Chromepet": LocalityData(
        city="Chennai", locality="Chromepet",
        land_rate_lo=5500, land_rate_hi=8550,
        apt_rate_lo=5700,  apt_rate_hi=8550,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Chromepet: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600044. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Pallavaram": LocalityData(
        city="Chennai", locality="Pallavaram",
        land_rate_lo=5700, land_rate_hi=9500,
        apt_rate_lo=5700,  apt_rate_hi=9000,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Pallavaram: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600043. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Pammal": LocalityData(
        city="Chennai", locality="Pammal",
        land_rate_lo=4550, land_rate_hi=7100,
        apt_rate_lo=4750,  apt_rate_hi=7100,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Pammal: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600075. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Anakaputhur": LocalityData(
        city="Chennai", locality="Anakaputhur",
        land_rate_lo=4000, land_rate_hi=6450,
        apt_rate_lo=4300,  apt_rate_hi=6650,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Anakaputhur: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600070. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Sembakkam": LocalityData(
        city="Chennai", locality="Sembakkam",
        land_rate_lo=4300, land_rate_hi=6850,
        apt_rate_lo=4750,  apt_rate_hi=7100,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Sembakkam: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600073. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Perungalathur": LocalityData(
        city="Chennai", locality="Perungalathur",
        land_rate_lo=3600, land_rate_hi=5700,
        apt_rate_lo=4000,  apt_rate_hi=5900,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Perungalathur: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600063. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Mudichur": LocalityData(
        city="Chennai", locality="Mudichur",
        land_rate_lo=3600, land_rate_hi=5700,
        apt_rate_lo=4000,  apt_rate_hi=5900,
        guideline_value=0,
        trend_12m="+7.5%",
        micro_context="Mudichur: South Chennai suburban. GST Road corridor. Airport proximity. First-home buyer zone.",
        infra_notes="Pincode 600048. Tambaram Corp zone. Tambaram Corporation.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Tambaram Corporation", "Chennai growth"],
        risk_factors=["Verify Tambaram Corp approvals before purchase"],
    ),

    "Chennai|Poonamallee": LocalityData(
        city="Chennai", locality="Poonamallee",
        land_rate_lo=4300, land_rate_hi=6650,
        apt_rate_lo=4300,  apt_rate_hi=6450,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Poonamallee: Poonamallee is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 600056. CMA zone. CMA - Tiruvallur.",
        data_confidence=82,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Thirumazhisai": LocalityData(
        city="Chennai", locality="Thirumazhisai",
        land_rate_lo=3050, land_rate_hi=4950,
        apt_rate_lo=3600,  apt_rate_hi=5500,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Thirumazhisai: Thirumazhisai is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 600124. CMA zone. CMA - Tiruvallur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Thiruverkadu": LocalityData(
        city="Chennai", locality="Thiruverkadu",
        land_rate_lo=3800, land_rate_hi=5900,
        apt_rate_lo=4300,  apt_rate_hi=6450,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Thiruverkadu: Thiruverkadu is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 600077. CMA zone. CMA - Tiruvallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Thiruninravur": LocalityData(
        city="Chennai", locality="Thiruninravur",
        land_rate_lo=1700, land_rate_hi=2850,
        apt_rate_lo=1900,  apt_rate_hi=2850,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Thiruninravur: Thiruninravur is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 602024. CMA zone. CMA - Tiruvallur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Veppampattu": LocalityData(
        city="Chennai", locality="Veppampattu",
        land_rate_lo=1550, land_rate_hi=2650,
        apt_rate_lo=1700,  apt_rate_hi=2750,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Veppampattu: Veppampattu is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 602024. CMA zone. CMA - Tiruvallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Tiruvallur Town": LocalityData(
        city="Chennai", locality="Tiruvallur Town",
        land_rate_lo=550, land_rate_hi=2200,
        apt_rate_lo=2100,  apt_rate_hi=3600,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Tiruvallur Town: Tiruvallur Town is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 602001. CMA zone. CMA - Tiruvallur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Ponneri": LocalityData(
        city="Chennai", locality="Ponneri",
        land_rate_lo=600, land_rate_hi=1700,
        apt_rate_lo=1700,  apt_rate_hi=2850,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Ponneri: Ponneri is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 601204. CMA zone. CMA - Tiruvallur.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Gummidipoondi": LocalityData(
        city="Chennai", locality="Gummidipoondi",
        land_rate_lo=300, land_rate_hi=1400,
        apt_rate_lo=1700,  apt_rate_hi=3050,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Gummidipoondi: Gummidipoondi is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 601201. CMA zone. CMA - Tiruvallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Periyapalayam": LocalityData(
        city="Chennai", locality="Periyapalayam",
        land_rate_lo=300, land_rate_hi=1300,
        apt_rate_lo=1550,  apt_rate_hi=2650,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Periyapalayam: Periyapalayam is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 601102. CMA zone. CMA - Tiruvallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Sevvapet": LocalityData(
        city="Chennai", locality="Sevvapet",
        land_rate_lo=350, land_rate_hi=1300,
        apt_rate_lo=1400,  apt_rate_hi=2500,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Sevvapet: Sevvapet is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 602025. CMA zone. CMA - Tiruvallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Uthukottai": LocalityData(
        city="Chennai", locality="Uthukottai",
        land_rate_lo=250, land_rate_hi=1200,
        apt_rate_lo=1300,  apt_rate_hi=2400,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Uthukottai: Uthukottai is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 602026. CMA zone. CMA - Tiruvallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Minjur": LocalityData(
        city="Chennai", locality="Minjur",
        land_rate_lo=850, land_rate_hi=2250,
        apt_rate_lo=2100,  apt_rate_hi=3600,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Minjur: Minjur is a residential locality in Chennai, CMA - Tiruvallur.",
        infra_notes="Pincode 601203. CMA zone. CMA - Tiruvallur.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Tiruvallur", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Sriperumbudur": LocalityData(
        city="Chennai", locality="Sriperumbudur",
        land_rate_lo=2850, land_rate_hi=4750,
        apt_rate_lo=3300,  apt_rate_hi=5200,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Sriperumbudur: Sriperumbudur is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 602105. CMA zone. CMA - Kanchipuram.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Oragadam": LocalityData(
        city="Chennai", locality="Oragadam",
        land_rate_lo=2650, land_rate_hi=4550,
        apt_rate_lo=3300,  apt_rate_hi=5200,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Oragadam: Oragadam is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 602105. CMA zone. CMA - Kanchipuram.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Mangadu": LocalityData(
        city="Chennai", locality="Mangadu",
        land_rate_lo=4000, land_rate_hi=6450,
        apt_rate_lo=4300,  apt_rate_hi=6450,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Mangadu: Mangadu is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 600122. CMA zone. CMA - Kanchipuram.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Kundrathur": LocalityData(
        city="Chennai", locality="Kundrathur",
        land_rate_lo=3800, land_rate_hi=6200,
        apt_rate_lo=4000,  apt_rate_hi=6200,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Kundrathur: Kundrathur is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 600069. CMA zone. CMA - Kanchipuram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Madanandapuram": LocalityData(
        city="Chennai", locality="Madanandapuram",
        land_rate_lo=4300, land_rate_hi=6650,
        apt_rate_lo=4750,  apt_rate_hi=6850,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Madanandapuram: Madanandapuram is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 600125. CMA zone. CMA - Kanchipuram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Thirumudivakkam": LocalityData(
        city="Chennai", locality="Thirumudivakkam",
        land_rate_lo=3600, land_rate_hi=5900,
        apt_rate_lo=4000,  apt_rate_hi=6200,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Thirumudivakkam: Thirumudivakkam is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 600132. CMA zone. CMA - Kanchipuram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Irungattukottai": LocalityData(
        city="Chennai", locality="Irungattukottai",
        land_rate_lo=2100, land_rate_hi=3800,
        apt_rate_lo=2650,  apt_rate_hi=4300,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Irungattukottai: Irungattukottai is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 602117. CMA zone. CMA - Kanchipuram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Walajabad": LocalityData(
        city="Chennai", locality="Walajabad",
        land_rate_lo=750, land_rate_hi=2100,
        apt_rate_lo=1550,  apt_rate_hi=2650,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Walajabad: Walajabad is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 631605. CMA zone. CMA - Kanchipuram.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Kanchipuram Town": LocalityData(
        city="Chennai", locality="Kanchipuram Town",
        land_rate_lo=1400, land_rate_hi=3800,
        apt_rate_lo=2400,  apt_rate_hi=4300,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Kanchipuram Town: Kanchipuram Town is a residential locality in Chennai, CMA - Kanchipuram.",
        infra_notes="Pincode 631501. CMA zone. CMA - Kanchipuram.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Kanchipuram", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Vandalur": LocalityData(
        city="Chennai", locality="Vandalur",
        land_rate_lo=3050, land_rate_hi=4950,
        apt_rate_lo=3600,  apt_rate_hi=5500,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Vandalur: Vandalur is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 600048. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Urapakkam": LocalityData(
        city="Chennai", locality="Urapakkam",
        land_rate_lo=3300, land_rate_hi=5200,
        apt_rate_lo=3600,  apt_rate_hi=5500,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Urapakkam: Urapakkam is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603210. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Guduvanchery": LocalityData(
        city="Chennai", locality="Guduvanchery",
        land_rate_lo=3050, land_rate_hi=4950,
        apt_rate_lo=3600,  apt_rate_hi=5500,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Guduvanchery: Guduvanchery is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603202. CMA zone. CMA - Chengalpattu.",
        data_confidence=82,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Maraimalai Nagar": LocalityData(
        city="Chennai", locality="Maraimalai Nagar",
        land_rate_lo=1150, land_rate_hi=2850,
        apt_rate_lo=3300,  apt_rate_hi=5200,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Maraimalai Nagar: Maraimalai Nagar is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603209. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Singaperumal Koil": LocalityData(
        city="Chennai", locality="Singaperumal Koil",
        land_rate_lo=1150, land_rate_hi=2850,
        apt_rate_lo=3300,  apt_rate_hi=5900,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Singaperumal Koil: Singaperumal Koil is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603204. CMA zone. CMA - Chengalpattu.",
        data_confidence=82,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Mahindra World City": LocalityData(
        city="Chennai", locality="Mahindra World City",
        land_rate_lo=1900, land_rate_hi=4300,
        apt_rate_lo=5350,  apt_rate_hi=7100,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Mahindra World City: Mahindra World City is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603004. CMA zone. CMA - Chengalpattu.",
        data_confidence=82,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Kattankulathur": LocalityData(
        city="Chennai", locality="Kattankulathur",
        land_rate_lo=2400, land_rate_hi=4750,
        apt_rate_lo=3600,  apt_rate_hi=5700,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Kattankulathur: Kattankulathur is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603203. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Potheri": LocalityData(
        city="Chennai", locality="Potheri",
        land_rate_lo=3300, land_rate_hi=5700,
        apt_rate_lo=4300,  apt_rate_hi=6650,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Potheri: Potheri is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603203. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Chengalpattu Town": LocalityData(
        city="Chennai", locality="Chengalpattu Town",
        land_rate_lo=1900, land_rate_hi=4300,
        apt_rate_lo=2850,  apt_rate_hi=4950,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Chengalpattu Town: Chengalpattu Town is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603001. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Padappai": LocalityData(
        city="Chennai", locality="Padappai",
        land_rate_lo=1400, land_rate_hi=3300,
        apt_rate_lo=2650,  apt_rate_hi=4550,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Padappai: Padappai is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 601301. CMA zone. CMA - Chengalpattu.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Mambakkam": LocalityData(
        city="Chennai", locality="Mambakkam",
        land_rate_lo=3600, land_rate_hi=5900,
        apt_rate_lo=4300,  apt_rate_hi=6450,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Mambakkam: Mambakkam is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 600127. CMA zone. CMA - Chengalpattu.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Ottiambakkam": LocalityData(
        city="Chennai", locality="Ottiambakkam",
        land_rate_lo=3600, land_rate_hi=5900,
        apt_rate_lo=4300,  apt_rate_hi=6450,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Ottiambakkam: Ottiambakkam is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 600130. CMA zone. CMA - Chengalpattu.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Ponmar": LocalityData(
        city="Chennai", locality="Ponmar",
        land_rate_lo=3050, land_rate_hi=4950,
        apt_rate_lo=3800,  apt_rate_hi=5700,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Ponmar: Ponmar is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 600048. CMA zone. CMA - Chengalpattu.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Thalambur": LocalityData(
        city="Chennai", locality="Thalambur",
        land_rate_lo=3800, land_rate_hi=5500,
        apt_rate_lo=5500,  apt_rate_hi=7600,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Thalambur: Thalambur is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 600130. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Padur": LocalityData(
        city="Chennai", locality="Padur",
        land_rate_lo=2650, land_rate_hi=4000,
        apt_rate_lo=5200,  apt_rate_hi=7400,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Padur: Padur is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603103. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Kelambakkam": LocalityData(
        city="Chennai", locality="Kelambakkam",
        land_rate_lo=3300, land_rate_hi=4750,
        apt_rate_lo=4750,  apt_rate_hi=6850,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Kelambakkam: Kelambakkam is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603103. CMA zone. CMA - Chengalpattu.",
        data_confidence=72,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Thiruporur": LocalityData(
        city="Chennai", locality="Thiruporur",
        land_rate_lo=2100, land_rate_hi=4300,
        apt_rate_lo=3300,  apt_rate_hi=5200,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Thiruporur: Thiruporur is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603110. CMA zone. CMA - Chengalpattu.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Kovalam ECR": LocalityData(
        city="Chennai", locality="Kovalam ECR",
        land_rate_lo=4300, land_rate_hi=8550,
        apt_rate_lo=4750,  apt_rate_hi=8100,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Kovalam ECR: Kovalam ECR is a residential locality in Chennai, CMA - Chengalpattu.",
        infra_notes="Pincode 603112. CMA zone. CMA - Chengalpattu.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Chengalpattu", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Arakkonam": LocalityData(
        city="Chennai", locality="Arakkonam",
        land_rate_lo=1150, land_rate_hi=2650,
        apt_rate_lo=2100,  apt_rate_hi=3600,
        guideline_value=0,
        trend_12m="+13.0%",
        micro_context="Arakkonam: Arakkonam is a residential locality in Chennai, CMA - Ranipet.",
        infra_notes="Pincode 631001. CMA zone. CMA - Ranipet.",
        data_confidence=60,
        demand_drivers=["Residential demand", "CMA - Ranipet", "Chennai growth"],
        risk_factors=["Verify CMA approvals before purchase"],
    ),

    "Chennai|Mylapore": LocalityData(
        city="Chennai", locality="Mylapore",
        land_rate_lo=16000, land_rate_hi=26000,
        apt_rate_lo=14000,  apt_rate_hi=20000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Mylapore: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600004. GCC zone. Zone 9 - Teynampet.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|R.A. Puram": LocalityData(
        city="Chennai", locality="R.A. Puram",
        land_rate_lo=22000, land_rate_hi=38000,
        apt_rate_lo=18000,  apt_rate_hi=26000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="R.A. Puram: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600028. GCC zone. Zone 9 - Teynampet.",
        data_confidence=72,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Boat Club": LocalityData(
        city="Chennai", locality="Boat Club",
        land_rate_lo=45000, land_rate_hi=90000,
        apt_rate_lo=28000,  apt_rate_hi=45000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Boat Club: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600028. GCC zone. Zone 9 - Teynampet.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    "Chennai|Poes Garden": LocalityData(
        city="Chennai", locality="Poes Garden",
        land_rate_lo=35000, land_rate_hi=70000,
        apt_rate_lo=24000,  apt_rate_hi=38000,
        guideline_value=0,
        trend_12m="+5.5%",
        micro_context="Poes Garden: Prime Central Chennai. T.Nagar retail, Nungambakkam commercial. Highest value GCC zone.",
        infra_notes="Pincode 600086. GCC zone. Zone 9 - Teynampet.",
        data_confidence=60,
        demand_drivers=["Residential demand", "Zone 9 - Teynampet", "Chennai growth"],
        risk_factors=["Verify GCC approvals before purchase"],
    ),

    # ── Bangalore (hardcoded — update manually or add blr_seed.csv) ──

    "Bangalore|Indiranagar": LocalityData(
        city="Bangalore", locality="Indiranagar",
        land_rate_lo=57000, land_rate_hi=67000,
        apt_rate_lo=21200,  apt_rate_hi=24800,
        guideline_value=15000,
        trend_12m="+8.4%",
        micro_context="Premium lifestyle locality. Expat demand. F&B hub.",
        infra_notes="Indiranagar Metro Station (Purple Line).",
        data_confidence=89,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Koramangala": LocalityData(
        city="Bangalore", locality="Koramangala",
        land_rate_lo=53400, land_rate_hi=62600,
        apt_rate_lo=19300,  apt_rate_hi=22700,
        guideline_value=14000,
        trend_12m="+9.8%",
        micro_context="Startup hub and premium residential nucleus.",
        infra_notes="Forum Mall. Metro Phase 3 proposed.",
        data_confidence=90,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Whitefield": LocalityData(
        city="Bangalore", locality="Whitefield",
        land_rate_lo=26700, land_rate_hi=31300,
        apt_rate_lo=12900,  apt_rate_hi=15100,
        guideline_value=9000,
        trend_12m="+12.2%",
        micro_context="Premier IT corridor. ITPB, Bagmane Tech Park.",
        infra_notes="Whitefield Metro Station (Purple Line, 2023).",
        data_confidence=88,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|HSR Layout": LocalityData(
        city="Bangalore", locality="HSR Layout",
        land_rate_lo=36800, land_rate_hi=43200,
        apt_rate_lo=14700,  apt_rate_hi=17300,
        guideline_value=11000,
        trend_12m="+10.9%",
        micro_context="Second startup hub. Planned grid. ORR access.",
        infra_notes="ORR frontage. Metro Phase 3 proposed.",
        data_confidence=86,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Marathahalli": LocalityData(
        city="Bangalore", locality="Marathahalli",
        land_rate_lo=22100, land_rate_hi=25900,
        apt_rate_lo=12000,  apt_rate_hi=14000,
        guideline_value=8500,
        trend_12m="+13.5%",
        micro_context="Highest-appreciating mid-segment. ORR junction.",
        infra_notes="Marathahalli Bridge. Metro proposed.",
        data_confidence=84,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Electronic City": LocalityData(
        city="Bangalore", locality="Electronic City",
        land_rate_lo=15200, land_rate_hi=17800,
        apt_rate_lo=10100,  apt_rate_hi=11900,
        guideline_value=7000,
        trend_12m="+15.0%",
        micro_context="Original IT hub. Infosys/Wipro/HCL. 200k+ jobs.",
        infra_notes="Electronic City Metro (Green Line, 2024).",
        data_confidence=83,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|JP Nagar": LocalityData(
        city="Bangalore", locality="JP Nagar",
        land_rate_lo=22000, land_rate_hi=26000,
        apt_rate_lo=10500,  apt_rate_hi=13500,
        guideline_value=8000,
        trend_12m="+7.8%",
        micro_context="Established South Bangalore. Good school belt.",
        infra_notes="NICE Road. Metro Phase 3.",
        data_confidence=80,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|BTM Layout": LocalityData(
        city="Bangalore", locality="BTM Layout",
        land_rate_lo=20000, land_rate_hi=24000,
        apt_rate_lo=9500,  apt_rate_hi=12500,
        guideline_value=7500,
        trend_12m="+12.1%",
        micro_context="Dense residential near Koramangala.",
        infra_notes="ORR. Silk Board junction.",
        data_confidence=78,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Hebbal": LocalityData(
        city="Bangalore", locality="Hebbal",
        land_rate_lo=24000, land_rate_hi=28000,
        apt_rate_lo=11000,  apt_rate_hi=14000,
        guideline_value=8500,
        trend_12m="+11.3%",
        micro_context="North Bangalore. Lake-front premium.",
        infra_notes="Hebbal flyover. BIAL road.",
        data_confidence=80,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Yelahanka": LocalityData(
        city="Bangalore", locality="Yelahanka",
        land_rate_lo=16000, land_rate_hi=20000,
        apt_rate_lo=8500,  apt_rate_hi=11500,
        guideline_value=6500,
        trend_12m="+14.2%",
        micro_context="Fast-growing North suburb near airport.",
        infra_notes="BIAL Airport 15km. Metro proposed.",
        data_confidence=76,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Sarjapur Road": LocalityData(
        city="Bangalore", locality="Sarjapur Road",
        land_rate_lo=24000, land_rate_hi=28000,
        apt_rate_lo=11500,  apt_rate_hi=14500,
        guideline_value=9000,
        trend_12m="+12.5%",
        micro_context="Major East Bangalore IT corridor.",
        infra_notes="ORR-Sarjapur junction.",
        data_confidence=82,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Bellandur": LocalityData(
        city="Bangalore", locality="Bellandur",
        land_rate_lo=26000, land_rate_hi=30000,
        apt_rate_lo=12500,  apt_rate_hi=15500,
        guideline_value=9500,
        trend_12m="+12.0%",
        micro_context="Premium East Bangalore. ORR access.",
        infra_notes="ORR. Bellandur lake.",
        data_confidence=80,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|KR Puram": LocalityData(
        city="Bangalore", locality="KR Puram",
        land_rate_lo=18000, land_rate_hi=22000,
        apt_rate_lo=9000,  apt_rate_hi=12000,
        guideline_value=7000,
        trend_12m="+13.5%",
        micro_context="East Bangalore affordable. Metro drives appreciation.",
        infra_notes="KR Puram Metro (Purple Line).",
        data_confidence=78,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

    "Bangalore|Hennur": LocalityData(
        city="Bangalore", locality="Hennur",
        land_rate_lo=16000, land_rate_hi=20000,
        apt_rate_lo=8500,  apt_rate_hi=11500,
        guideline_value=6500,
        trend_12m="+13.2%",
        micro_context="North-East emerging corridor.",
        infra_notes="Metro Phase 2 proposed. ORR.",
        data_confidence=76,
        demand_drivers=["Residential demand", "Bangalore IT growth"],
        risk_factors=["Verify BBMP khata and E-khata before purchase"],
    ),

}

import difflib as _difflib
import logging as _logging
_db_logger = _logging.getLogger("valuprop.db")

def get_locality(city: str, locality: str) -> "Optional[LocalityData]":
    # Split all comma-separated parts (e.g. "Vaishnavi nagar, Tirumullaivoyal"
    # gives ["Vaishnavi nagar", "Tirumullaivoyal"]).
    parts = [p.strip() for p in locality.split(",") if p.strip()]
    loc_primary = parts[0]

    # Step 1: exact key match — try full string and all parts
    for loc_try in ([locality] + parts):
        data = LOCALITY_DB.get(f"{city}|{loc_try}")
        if data:
            return data

    # Step 2: case-insensitive substring match — try all parts
    city_lower = city.lower()
    for loc_try in ([locality] + parts):
        loc_lower = loc_try.lower()
        for k, v in LOCALITY_DB.items():
            if city_lower in k.lower() and loc_lower in k.lower():
                return v

    # Step 3: fuzzy match — try ALL parts, pick the best-scoring match.
    city_entries = {
        k.split("|")[1].lower(): (k, v)
        for k, v in LOCALITY_DB.items()
        if city_lower in k.lower()
    }
    if city_entries:
        best_score = 0.0
        best_val   = None
        best_input = loc_primary

        for part in parts:
            part_lower = part.lower()
            m = _difflib.get_close_matches(
                part_lower, city_entries.keys(), n=1, cutoff=0.62
            )
            if m:
                score = _difflib.SequenceMatcher(None, part_lower, m[0]).ratio()
                if score > best_score:
                    best_score = score
                    _, best_val = city_entries[m[0]]
                    best_input  = part

        if best_val:
            _db_logger.info(
                f"Fuzzy match: '{best_input}' (from '{locality}') → '{best_val.locality}' "
                f"({round(best_score * 100)}% similarity)"
            )
            return best_val

    return None

def get_confidence_label(score: int) -> str:
    if score >= 85: return "Strong"
    if score >= 70: return "Good"
    if score >= 60: return "Fair"
    return "Weak"

def list_localities(city: str) -> list[str]:
    return [v.locality for k, v in LOCALITY_DB.items() if v.city.lower() == city.lower()]

from typing import Optional