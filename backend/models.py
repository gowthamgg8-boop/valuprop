...
class AuditEvent(Base):
    """
    OWASP A09 — Security audit log.

    A durable, queryable trail of security-relevant events: payment
    verifications, signature failures, access-denied responses,
    rate-limit hits, webhook activity. Distinct from the `events`
    table above, which is product/funnel analytics — this one is for
    security investigation and incident response.
    """
    __tablename__ = "audit_events"

    id         = Column(Integer, primary_key=True, index=True)
    event_type = Column(String(50), index=True)   # e.g. payment_verified, access_denied, signature_failure
    severity   = Column(String(10), index=True)   # info | warning | critical
    ip         = Column(String(64), nullable=True)
    detail     = Column(JSON, nullable=True)       # structured context (no raw PII)
