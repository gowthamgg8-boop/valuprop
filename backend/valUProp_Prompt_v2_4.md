# valUProp.in — AI Residential Real Estate Valuation Prompt (v2.4)
...
- Locality-level per-sqft rates (range, average, recent transaction data)
...
- Resale stock older than 10 years: subtract 20–35% from “average ₹/sqft”
- Quoted listing prices: subtract 10–15% for likely closing price
...
*This is v2.4 of the valUProp valuation prompt. Updated June 2026. Builds on v2.3 (explicit Income/Rental-Yield Support Factor and Rental-Yield Cross-Check table). **v2.4 fixes the structural wiring of the Connectivity Factor**: it is now applied exactly once, at Step 5, as an explicit multiplicative line for every property type — removed from the land Step-4 adjustment list (eliminating the double-count risk), forward-referenced in all three base-value formulas so it can no longer be silently skipped for apartments/houses/villas, broken out into named sub-components (corridor, metro/rail, road frontage, employment access) instead of a generic "location adjustment", and reconciled across the Methodology and Output-Structure step numbering (previously Step 5 vs. Step 3). No other valuation logic changed.*
