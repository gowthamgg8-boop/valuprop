...
# ═══════════════════════════════════════════════════════════════════
# Email is personal data (DPDP Act 2023). It must never be written to
# application logs in clear text. This produces a masked form that is
# still useful for debugging (you can tell two users apart) without
# exposing the actual address. Defined locally rather than imported
# from main.py to avoid a circular import.
# ═══════════════════════════════════════════════════════════════════
...
async def send_report_email(
    ...
    try:
        ...
        # OWASP A02 — mask recipient email in logs
        logger.info(f"Email sent: to={mask_email(to_email)} id={email.get('id')} val={valuation_id}")
        return True

    except Exception as e:
        # OWASP A02 — mask recipient email in logs
        logger.error(f"Email failed: to={mask_email(to_email)} val={valuation_id} error={e}")
        return False
