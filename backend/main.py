...
async def verify_payment(req: VerifyRequest, bg: BackgroundTasks, db=Depends(get_db)):
    ...
    In DEMO mode (order_id starts with 'demo_'), skip Razorpay and create
    a payment record on-the-fly so the report can be generated.
    """
    is_demo = req.razorpay_order_id.startswith("demo_") or "YOUR" in (RAZORPAY_KEY_SECRET or "")
    ...
    if not payment:
        if is_demo:
            # DEMO MODE — create a payment record on-the-fly
            free_val = db.query(models.Valuation).filter(models.Valuation.id == req.valuation_id).first()
            if not free_val:
                raise HTTPException(404, "Valuation not found")
            payment = models.Payment(
                valuation_id       = free_val.id,
                razorpay_order_id  = req.razorpay_order_id,
                amount             = REPORT_PRICE_PAISE,
                status             = "pending",
            )
            db.add(payment); db.flush()
            logger.info(f"DEMO payment auto-created: order={req.razorpay_order_id} val={req.valuation_id}")
        else:
            raise HTTPException(404, "Payment not found")

    # Verify signature (skip in demo)
    if not is_demo and RAZORPAY_KEY_SECRET and "YOUR" not in RAZORPAY_KEY_SECRET:
...
def _to_property_input(area_data: dict):
    ...
    # Map form fields → PropertyInput fields
    age_str = area_data.get("age_apartment") or area_data.get("age_house") or area_data.get("age_villa") or ""
    bhk     = area_data.get("bhk") or ""
    floor   = area_data.get("floor_info") or area_data.get("floor") or ""
    ...
    return PropertyInput(
        ...
        prop_name    = area_data.get("project_name") or area_data.get("building_name") or "",
        bhk          = bhk,
        carpet_area  = area_data.get("carpet_area") or area_data.get("carpet_area_sqft"),
        builtup_area = area_data.get("builtup_area"),
        super_builtup= area_data.get("super_builtup"),
        floor_info   = floor,
        age_apt      = age_str if (area_data.get("type") == "Apartment") else "",
...
async def _run_paid_report(valuation_id: int, area_data: dict, user_email: str):
    ...
    try:
        val = db.query(models.Valuation).filter(models.Valuation.id == valuation_id).first()
        prop_input = _to_property_input(area_data)
        paid_report = await valuation_engine.generate_detailed_report(prop_input)
