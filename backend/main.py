...
# ═══════════════════════════════════════════════════════════════════
# ONE-TIME DB RESET — REMOVE THIS BLOCK AFTER DEPLOY
# ═══════════════════════════════════════════════════════════════════
# Set DB_RESET_ON_STARTUP=yes in Render env vars to drop & recreate
# the public schema on startup. Used to apply the access_token column
# addition (OWASP A01 fix) without psql shell access.
#
# After the reset has run once successfully:
#   1. Delete the DB_RESET_ON_STARTUP env var in Render
#   2. Push the regular main.py (without this block)
# ═══════════════════════════════════════════════════════════════════
if os.getenv("DB_RESET_ON_STARTUP", "").lower() == "yes":
    from sqlalchemy import text
    logger.warning("DB_RESET_ON_STARTUP=yes — wiping public schema NOW")
    with engine.begin() as conn:
        conn.execute(text("DROP SCHEMA public CASCADE"))
        conn.execute(text("CREATE SCHEMA public"))
    logger.warning("Schema dropped + recreated. Recreating tables...")
    models.Base.metadata.create_all(bind=engine)
    logger.warning("DB reset complete. REMOVE DB_RESET_ON_STARTUP env var NOW.")
...
async def verify_payment(req: VerifyRequest, bg: BackgroundTasks, db=Depends(get_db)):
    ...
    # Demo mode skips signature check
    if req.razorpay_order_id.startswith("demo_order_"):
