...
def _generate_reportlab(report: dict, area: dict, val_id: int) -> bytes:
    ...
    s_normal  = sty("vp_normal",  fontSize=9,  leading=14, textColor=C_TEXT)
    s_muted   = sty("vp_muted",   fontSize=8,  leading=12, textColor=C_MUTED)
    s_bold    = sty("vp_bold",    fontSize=9,  leading=14, textColor=C_TEXT,  fontName="Helvetica-Bold")
    s_h1      = sty("vp_h1",      fontSize=18, leading=24, textColor=C_BLUE,  fontName="Helvetica-Bold")
    s_h2      = sty("vp_h2",      fontSize=11, leading=16, textColor=C_TEXT,  fontName="Helvetica-Bold")
    s_label   = sty("vp_label",   fontSize=7,  leading=10, textColor=C_BLUE,  fontName="Helvetica-Bold")
    s_white   = sty("vp_white",   fontSize=9,  leading=14, textColor=colors.white)
    s_wbold   = sty("vp_wbold",   fontSize=14, leading=20, textColor=colors.white, fontName="Helvetica-Bold")
    s_wsmall  = sty("vp_wsmall",  fontSize=8,  leading=11, textColor=colors.HexColor("#CBD5E1"))
    s_wmeta   = sty("vp_wmeta",   fontSize=8,  leading=12, textColor=colors.HexColor("#CBD5E1"))
    ...
    # ── HEADER ───────────────────────────────────────────────────
    header_data = [[
        Paragraph("<b>valUProp<font color='#FFD580'>.in</font></b><br/>"
                  "<font size='7' color='#94A3B8'>Independent Market Valuation</font>", s_wbold),
        Paragraph(f"<font color='#94A3B8'>Date: {date_str}<br/>Ref: {ref}<br/>Type: {prop_type}</font>", s_wmeta),
    ]]
    ...
    # ── VALUE BANNER ─────────────────────────────────────────────
    banner_data = [[
        [Paragraph("PROPERTY", s_label), Spacer(1,3), Paragraph(address, s_h2)],
        [Paragraph("ESTIMATED MARKET VALUE", s_label), Spacer(1,3),
         Paragraph(val_range, s_h1),
         Paragraph("Excl. registration charges &amp; taxes", s_muted),
         Spacer(1,4),
         Paragraph(f"<b>Confidence: {conf}% \u2014 {conf_label}</b>",
                   sty("conf", fontSize=8, leading=11,
                       textColor=colors.white, fontName="Helvetica-Bold",
                       backColor=C_CONF, borderPadding=4)),
        ],
    ]]
    banner_tbl = Table(banner_data, colWidths=[W*0.45, W*0.55])
    ...
    if conf < 70:
        warn_tbl = Table([[
            Paragraph(f"\u26a0\ufe0f  Confidence score is below 70%. We recommend consulting a registered "
                      f"valuer before making any financial decisions on this property.", s_normal)
        ]], colWidths=[W])
    ...
    for letter in ["A","B","C","D","E","F","G"]:
        ...
        is_opinion = letter == "E"
        bg_color   = C_LIGHT if is_opinion else C_BG

        block = []

        # Section header row
        hdr_data = [[
            Paragraph(letter, sty("badge", fontSize=10, leading=14,
                                  textColor=colors.white, fontName="Helvetica-Bold",
                                  backColor=C_BLUE, borderPadding=3)),
            Paragraph(title, s_h2),
        ]]
        hdr_tbl = Table(hdr_data, colWidths=[18, W-18])
        hdr_tbl.setStyle(TableStyle([
            ("BACKGROUND", (0,0), (-1,-1), bg_color if is_opinion else C_BG),
            ("VALIGN",     (0,0), (-1,-1), "MIDDLE"),
            ("TOPPADDING", (0,0), (-1,-1), 8),
            ("BOTTOMPADDING",(0,0),(-1,-1),8),
        ...
        if letter == "C":
            ...
            for k, fk in [("Land Rate Range","land_rate_range"),
                ...
                v = sec.get(fk,"")
                if v:
                    rows.append([Paragraph(k, s_muted), Paragraph(v, s_bold)])
            if rows:
        ...
        # Section D data table
            ...
            for k, fk in [("Land Value","land_value"),
                           ("Building Value (Depr.)","building_value"),
                           ("Location Adjustments","adjustments")]:
                v = sec.get(fk,"")
                if v:
                    rows.append([Paragraph(k, s_muted), Paragraph(v, s_bold)])
            if rows:
        ...
        # Section E value range
            ...
            if vr:
                vr_tbl = Table([[Paragraph(vr, s_h1)]], colWidths=[W])
        ...
        # Section F risk list
            ...
            if risks:
                for r in risks:
                    if r.strip():
                        rt = Table([[
                            Paragraph("\u2022", sty("rb", fontSize=12, leading=14,
                                                   textColor=C_ACCENT, fontName="Helvetica-Bold")),
                            Paragraph(r.lstrip("\u2022- "), s_normal),
                        ]], colWidths=[12, W-12])
        ...
        # Content paragraph
        if content:
            ct = Table([[Paragraph(content, s_normal)]], colWidths=[W])
        ...
        card = Table([[b] for b in block], colWidths=[W])
        card.setStyle(TableStyle([
            ("BOX",(0,0),(-1,-1),1,C_BLUE if is_opinion else C_BORDER),
            ("BACKGROUND",(0,0),(-1,-1),C_LIGHT if is_opinion else colors.white),
    ...
    if comps:
        comp_rows = [[
            Paragraph("Property", s_bold),
            Paragraph("Price Signal", s_bold),
            Paragraph("Source", s_bold),
        ]]
        for c in comps:
            comp_rows.append([
                Paragraph(c.get("description",""), s_normal),
                Paragraph(c.get("price_signal",""), sty("cs", fontSize=9, leading=14,
                                                         textColor=C_BLUE, fontName="Helvetica-Bold")),
                Paragraph(c.get("source",""), s_muted),
            ])
    ...
    story.append(Paragraph(
        f"valUProp.in \u00b7 AI-Powered Property Valuation \u00b7 India \u00b7 {ref} \u00b7 {date_str} \u00b7 "
        "Not a statutory or bank-certified valuation", s_muted))
    ...
    def _footer(canvas, doc):
        ...
        canvas.drawString(16*mm, 12*mm, f"{ref} \u00b7 {date_str} \u00b7 valUProp.in")
        canvas.drawRightString(A4[0]-16*mm, 12*mm, f"Page {doc.page}")
        canvas.restoreState()
