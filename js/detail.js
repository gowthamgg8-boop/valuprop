...
async function pollForReport(valuation_id) {
  try {
    ...
    if (resp.status === 401 || resp.status === 403) {
      showError('Access denied. Your session may have expired — please re-purchase the report or email support@valuprop.in with ref: VUP-' + valuation_id);
      return;
    }
    ...
    if (data.status === 'pending') {
      ...
      if (pollCount >= MAX_POLLS) {
        showError('Report generation is taking longer than expected. Please email support@valuprop.in with ref: VUP-' + valuation_id);
        return;
      }
    ...
    if (data.status === 'error') {
      showError(data.message || 'Report generation failed. Please contact support@valuprop.in');
      return;
    }
